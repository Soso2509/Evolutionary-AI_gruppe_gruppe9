...
def calculate_monthly_returns(file_path):
    ...
    monthly_prices_first = df['Adj Close'].resample('M').first()  # Første dag i måneden
    monthly_prices_last = df['Adj Close'].resample('M').last()   # Siste dag i måneden
...
returns_df.to_csv('monthly_returns_all_stocks.csv')

print("Månedlige avkastninger er beregnet og lagret i 'monthly_returns_all_stocks_corrected.csv'.")
